...
vaccinations_S[, 5] <-  booster_first[i] * sum(S[i, 3:4]) + booster_second[i] * sum(S[i, (j + 1):N_vaccine])
vaccinations_S[, 6:N_vaccine] <- -booster_second[i] * S[i, j]
...
deriv(E1[, 2:N_vaccine]) <- (lambda[i] * vaccine_efficacy_infection_t[i, j] * S[i, j]) - (gamma_E_t * E1[i, j]) - gamma_vaccine_t[j] * E1[i, j] + vaccinations_E1[i,j] + gamma_vaccine_t[j - 1] * E1[i, j - 1]
...
vaccinations_E1[, 5] <-  booster_first[i] * sum(E1[i, 3:4]) + booster_second[i] * sum(E1[i, (j + 1):N_vaccine])
vaccinations_E1[, 6:N_vaccine] <- -booster_second[i] * E1[i, j]
...
deriv(E2[, 2:N_vaccine]) <- (gamma_E_t * E1[i, j]) - (gamma_E_t * E2[i, j]) - gamma_vaccine_t[j] * E2[i, j] + vaccinations_E2[i,j] + gamma_vaccine_t[j - 1] * E2[i, j - 1]
...
vaccinations_E2[, 5] <-  booster_first[i] * sum(E2[i, 3:4]) + booster_second[i] * sum(E2[i, (j + 1):N_vaccine])
vaccinations_E2[, 6:N_vaccine] <- -booster_second[i] * E2[i, j]
...
deriv(IMild[, 2:N_vaccine]) <- (gamma_E_t * E2[i, j] * (1 - prob_hosp_t_mult[i, j])) - (gamma_IMild_t * IMild[i, j]) - gamma_vaccine_t[j] * IMild[i, j] + gamma_vaccine_t[j - 1] * IMild[i, j - 1]
...
deriv(R1[, 2:N_vaccine]) <- (gamma_rec * IRec2[i, j]) + (gamma_IMild_t * IMild[i, j]) + (gamma_get_ox_survive_t * IOxGetLive2[i, j]) + (gamma_not_get_ox_survive * IOxNotGetLive2[i, j]) + (gamma_not_get_mv_survive * IMVNotGetLive2[i, j]) - (gamma_R_t * R1[i, j]) - gamma_vaccine_t[j] * R1[i, j] + vaccinations_R1[i,j] + gamma_vaccine_t[j - 1] * R1[i, j - 1]
...
vaccinations_R1[, 5] <-  booster_first[i] * sum(R1[i, 3:4]) + booster_second[i] * sum(R1[i, (j + 1):N_vaccine])
vaccinations_R1[, 6:N_vaccine] <- -booster_second[i] * R1[i, j]
...
vaccinations_R2[, 5] <-  booster_first[i] * sum(R2[i, 3:4]) + booster_second[i] * sum(R2[i, (j + 1):N_vaccine])
vaccinations_R2[, 6:N_vaccine] <- -booster_second[i] * R2[i, j]
...
deriv(ICase1[, 2:N_vaccine]) <- (gamma_E_t * E2[i, j] * prob_hosp_t_mult[i, j]) - (gamma_ICase_t * ICase1[i, j]) - gamma_vaccine_t[j] * ICase1[i, j] + gamma_vaccine_t[j - 1] * ICase1[i, j - 1]

deriv(ICase2[, 1]) <- (gamma_ICase_t * ICase1[i, j]) - (gamma_ICase_t * ICase2[i, j]) - gamma_vaccine_t[j] * ICase2[i, j]
deriv(ICase2[, 2:N_vaccine]) <- (gamma_ICase_t * ICase1[i, j]) - (gamma_ICase_t * ICase2[i, j]) - gamma_vaccine_t[j] * ICase2[i, j] + gamma_vaccine_t[j - 1] * ICase2[i, j - 1]
...
deriv(IOxGetLive1[, 2:N_vaccine]) <- (gamma_ICase_t * ICase2[i, j] * (1 - prob_severe_multi[i]) * p_oxygen * (1 - prob_non_severe_death_treatment[i])) - (gamma_get_ox_survive_t * IOxGetLive1[i, j]) - gamma_vaccine_t[j] * IOxGetLive1[i, j] + gamma_vaccine_t[j - 1] * IOxGetLive1[i, j - 1]
...
deriv(IOxNotGetLive1[, 2:N_vaccine]) <- (gamma_ICase_t * ICase2[i, j] * (1 - prob_severe_multi[i]) * (1 - p_oxygen) * (1 - prob_non_severe_death_no_treatment[i])) - (gamma_not_get_ox_survive * IOxNotGetLive1[i, j]) - gamma_vaccine_t[j] * IOxNotGetLive1[i, j] + gamma_vaccine_t[j - 1] * IOxNotGetLive1[i, j - 1]
...
deriv(IMVGetLive1[, 2:N_vaccine]) <- (gamma_ICase_t * ICase2[i, j] * prob_severe_multi[i] * p_ventilation * (1 - prob_severe_death_treatment[i])) - (gamma_get_mv_survive_t * IMVGetLive1[i, j]) - gamma_vaccine_t[j] * IMVGetLive1[i, j] + gamma_vaccine_t[j - 1] * IMVGetLive1[i, j - 1]
...
deriv(IMVNotGetLive1[, 2:N_vaccine]) <- (gamma_ICase_t * ICase2[i, j] * prob_severe_multi[i] * (1 - p_ventilation) * (1 - prob_severe_death_no_treatment[i])) - (gamma_not_get_mv_survive * IMVNotGetLive1[i, j]) - gamma_vaccine_t[j] * IMVNotGetLive1[i, j] + gamma_vaccine_t[j - 1] * IMVNotGetLive1[i, j - 1]
...
dim(vaccine_booster_initial_coverage) <- N_age

vaccine_booster_follow_up_coverage[] <- user()
dim(vaccine_booster_follow_up_coverage) <- N_age
...
#   sum(R1[i,j:N_vaccine]) + sum(R2[i,j:N_vaccine])
dim(dose_pops) <- c(N_age, 6) # 1 is everyone alive, 2 is all with first dose, 3 is all with second dose, 4 is all initial waned, 5 is all with first booster dose, 6 is all who've waned
...
dim(vaccination_cov) <- c(N_age, 5)
#1: Unvaccinated, 2: First Dose, 3: Second Dose, 4:Waned Second Dose, 5:Boosted
...
# deriv(vaccines_cumu[, 1]) <- primary_first[i] * (S[i, j] + E1[i, j] + E2[i, j] + R1[i, j] + R2[i, j])
# deriv(vaccines_cumu[, 2]) <- primary_second * (sum(S[i, 2:3]) + sum(E1[i, 2:3]) + sum(E2[i, 2:3]) + sum(R1[i, 2:3]) + sum(R2[i, 2:3]))
...
output(booster_doses_given[]) <- booster_first[i] * (sum(S[i, 3:4]) + sum(E1[i, 3:4]) +
                                           sum(E2[i, 3:4]) + sum(R1[i, 3:4]) + sum(R2[i, 3:4])) +
  booster_second[i] * (sum(S[i, 6:N_vaccine]) + sum(E1[i, 6:N_vaccine]) +
                       sum(E2[i, 6:N_vaccine]) + sum(R1[i, 6:N_vaccine]) + sum(R2[i, 6:N_vaccine]))
...
output(vaccinated_second_waned[]) <- vaccination_cov[i, 4]
dim(vaccinated_second_waned) <- N_age
# Vaccinated booster Dose
output(vaccinated_booster_dose[]) <- dose_pops[i, 5]
dim(vaccinated_booster_dose) <- N_age
# Waned
output(vaccinated_booster_waned[]) <- dose_pops[i, 6]
dim(vaccinated_booster_waned) <- N_age
