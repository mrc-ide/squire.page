...
get_Rt <- function(model_out){
  ...
  #check if excess model (replace with something better)
  if("week_start" %in% names(model_out$pmcmc_results$inputs$data)){
    model_out$pmcmc_results$inputs$data$date <- model_out$pmcmc_results$inputs$data$week_start
